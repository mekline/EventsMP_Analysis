...
%stuff I know about. I've moved all the common parameters we usually set up to the top of the 
%script; this template assumes you are finding the top %10 voxels in each participant; change
%it in the big struct if you want something else. It also assumes you'll provide a parcel file
%(manual ROI option)

%On run, matlab asks you for some stuff; we usually set Minimal ROI-level overlap to 0 and Explicit masking to None.  Those should be
...
firstlevel_crit = 'firstlevel_langlocSN' % path  for your critical exp - in this case measuring the langloc response itself
...
MyOutputFolder = '/mindhive/evlab/u/mekline/Desktop/EventsMP/Toolbox/REORGTEMPLATE_20160106_RESULTS' %Where should the results wind up?

loc_cons = {{'S-N'}} %Which contrast used to localize issROIs?
crit_cons = {{'S-N' }} %What contrasts of the crit. experiment do we want to measure there? In this case they match bc we are measuring each person's lang centers response to lang stimuli!
